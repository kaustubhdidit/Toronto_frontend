...
const BillBoard = ({ props }) => {
  ...
  const editVac=async (i)=>{
    ...
    // console.log(price)
    // API Call
    const response = await fetch(`https://totento-backend.onrender.com/api/v1/room/vac/${i._id}`, {
      method: "PUT", 
      headers: {
  ...
  useEffect(() => {
    ...
    axios.get('https://totento-backend.onrender.com/api/v1/room/my', {
      params: {
    ...
    <div>
      ...
      <Modal show={showModal} onHide={handleClose}>
        <Modal.Header style={{display:'flex', border:'1px solid gray',  justifyContent:'center', backgroundColor:'black'}}>
          <Modal.Title style={{fontWeight:'bolder', color:'white'}}>Edit Room Info.</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {/* Your form content */}
          <form className='my-3'>
            ...
            <div className="category" key={i._id}>
              ...
              <div className="thre">
                <div className="left">
                  ...
                  <p style={{ fontWeight: 'bold', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{i.title}</p>
                </div>
              </div>
              <div className="form-check form-switch" style={{ position: 'relative', left: '0.5cm', bottom: '0.4cm' }}>
                ...
                <label className="form-check-label" htmlFor={`flexSwitchCheck${i._id}`}>
